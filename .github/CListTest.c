...
/* ------------------------------------- */
/* Tests para listas circulares (List_int) */
/* ------------------------------------- */

START_TEST(test_list_create) {
    List_int* list = list_int_create();
    ck_assert_ptr_nonnull(list);
    ck_assert_ptr_nonnull(list->head);  // Cabeza debe existir en una lista circular
    ck_assert_ptr_nonnull(list->tail);  // Cola también debe existir
    ck_assert_ptr_eq(list->head, list->tail);  // En una lista circular, cabeza y cola deben ser iguales en la creación
    ck_assert_uint_eq(list_int_length(list), 0);
...
START_TEST(test_insert_and_get) {
    ...
    list_int_insert(list, 10, 0);  // Inserta al principio
    list_int_insert(list, 30, 1);  // Inserta al final
    list_int_insert(list, 20, 1);  // Inserta en el medio
